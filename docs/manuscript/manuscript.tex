...
Attempting to replicate a study is a means of critically assessing whether the evidence provided by the researchers that conducted the study support the claims they make and their explanation of a phenomenon. 
Replications confront existing explanations with new evidence by retesting prior claims using new data, while typically controlling for procedural differences by conducting similar analyses.
Replications can be designed to serve epistemic functions and to test several forms of validity.
Attempts to reproduce prior research also retest claims, but do so using the same data and the same, or very similar, analyses. 
The central focus a reproduction attempt is the verification of claims through the examination of how a study was conducted and whether the design and execution of that study support the results presented.
While the results of a reproduction or replication do not provide conclusive evidence for or against a claim, those that produce outcomes consistent with prior studies typically increase confidence in a claim and the theories underlying it \citep{earp2015, nichols2021}. 
When prior results cannot be recreated, the claims of the prior study, the data and methods used, and the current understanding of the phenomena may be brought into question \citep{christensen2019, NASEM2019}.
...
However, those studies also largely focus on testing the conclusion and internal validity of past studies through computational reproduction. 
...
With those intellectual foundations, it remains unclear how current geographic researchers view replication and the role of the practice in the knowledge accumulation process.
...
Broadly defined, a replication study is any study that has at least one outcome that would be considered diagnostic evidence of a claim from prior research, and can be used to assess the validity of prior claims \citep{nosek2020}.
More narrowly, replication studies are differentiated by the alterations they make to a study, which is linked to the epistemological function they are intended to serve.
By altering different aspects of a study a researcher attempting a replication can use their finding to potentially assess (1) conclusion, (2) internal, (3) construct, and (4) external validity \citep[see][]{schmidt2009, gomez2010replications, radder2003, radder2012}.

OLD: Replications provide evidence of the credibility of claims across conditions that either naturally or are purposefully manipulated to differ from the original study. 
...
This tendency toward criteria may suggest that replication as an epistemological tool is not be at the forefront of geographic researchers' thinking about replication.
...
Building on the differences that exist between research traditions in geography, it may be fruitful to further examine how researcher working in different subfield precieve and use replication in the work. 
Overall, the differences in responses we observed across subfields and methodological approaches was small, but this finding should be interpreted with caution given sizes of our subfield samples.
However, it is sensible that responses to questions about barriers to replicability and motivations to attempt replications would be similar between groups because researchers across the discipline work in similar academic incentive systems. 
One notable exception to this consistency was our finding that human geographers and qualitative researchers were disproportionately likely to provide 'I do not know' responses across survey questions and to question the epistemological role of replication in their subfield. 
...
Our work provides an initial measurement of geographers definitions of replicability, beliefs about what factors affects the chances of replicating a study, motivations to attempt replication studies, and experiences conducting replications. 
